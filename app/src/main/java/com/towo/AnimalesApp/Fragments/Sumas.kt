...
import com.towo.AnimalesApp.databinding.SumasJuegoBinding
import java.util.*




class Sumas : Fragment(), RewardedVideoListener {
    ...
    private var _binding: SumasJuegoBinding? = null
    ...
    private lateinit var rdmButton: IntArray
    ...
    override fun onCreateView(
        ...
        _binding = SumasJuegoBinding.inflate(layoutInflater, container, false)
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.myToolbar.setNavigationOnClickListener {
            Navigation.findNavController(view).navigate(R.id.action_sumas2_to_seleccion)
        }
    ...
    private fun dialogGameOver(){
        context?.let {
            ...
            homeButton.setOnClickListener {
                alertDialog.dismiss()
                view?.let { it1 -> Navigation.findNavController(it1).navigate(R.id.action_sumas2_to_seleccion) }
            }
            ...
            when (choiceButton) {
                0 -> {
                    activity?.runOnUiThread {
                        ...
                        binding.R2.text = rdmButton[1].toString()
                        binding.R3.text = rdmButton[2].toString()
                    }
                }
                1 -> {
                    activity?.runOnUiThread {
                        binding.R2.text = resultado.toString()
                        binding.R1.text = rdmButton[1].toString()
                        binding.R3.text = rdmButton[2].toString()
                    }
                }
                else -> {
                    activity?.runOnUiThread {
                        ...
                        binding.R2.text = rdmButton[1].toString()
                        binding.R1.text = rdmButton[2].toString()
                    }
                }
            }
        }.start()
    ...
    private fun threadNotRepeat() {
        ...
        Thread {
            ...
            var contador = 1
            rdmButton[0] = resultado
            while (contador < rdmButton.size) {
                val temp = random.nextInt(50)
                var i = 0
                while (i < rdmButton.size - 1) {
                    if (temp == rdmButton[i]) {
                        break
                    }
                    i++
